...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        //Pagos
        $payments = Movement::select('amount')->where('movement_type', 'D')
                            ->where('type', 'P')
                            ->whereYear('date', '=', $current_year)
                            ->whereMonth('date', '=', $current_month)->get();
        //Descuentos
        $discounts = Movement::select('amount')->where('movement_type', 'D')
                            ->where('type', 'D')
                            ->whereYear('date', '=', $current_year)
                            ->whereMonth('date', '=', $current_month)->get();
        //Recibos del Mes
        $invoices_month = Invoice::select('status','total')->whereYear('date', '=', $current_year)
                            ->whereMonth('date', '=', $current_month)->get();

        //Recibos del Año
        $invoices_year = Invoice::select('status','total')->whereYear('date', '=', $current_year)->get();

        //Debitos del Año
        $debits_year = Movement::select('type','amount')->whereYear('date', '=', $current_year)
...
*/
    ...
    public function invoices_by_neighborhood($current_year){
        $output="";
        $i=1;
        $contracts = Contract::select('neighborhood')->distinct()->groupBy('neighborhood')->get();
        $max_i =  $contracts->count();
        foreach ($contracts as $contract) {
            $invoices = Invoice::select('total')->whereYear('date', '=', $current_year)
                                ->whereHas('Contract',
    ...
    public function incomes_by_neighborhood($current_year){
        ...
        foreach ($contracts as $contract) {
            //Pagos (no incluye descuentos)
            $movements = Movement::select('amount')->whereYear('date', '=', $current_year)
