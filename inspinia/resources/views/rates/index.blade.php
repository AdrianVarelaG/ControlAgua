            ...
            @if($rates->count())
                <div class="table-responsive">
                    ...
                    <tr>
                        <th></th>
                        <th>Tarifa</th>
                        <th>Monto/m3 {{ Session::get('coin') }}</th>
                        <th>Registro</th>
                        <th>Observación</th>
                    </tr>
                    </thead>
                          ...
                          @if($rate->status == "A")
                            <div class="input-group-btn">
                                ...
                                <ul class="dropdown-menu">
                                    <li><a href="{{ route('rates.edit', Crypt::encrypt($rate->id)) }}"><i class="fa fa-pencil"></i> Editar</a></li>
                                    <li><a href="{{ route('rates.status', Crypt::encrypt($rate->id)) }}"><i class="fa fa-ban"></i> Deshabilitar</a></li>
                                    <li class="divider"></li>
                                    <li>
                                        <!-- href para eliminar registro -->                            
                                        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                        <form action="{{ route('rates.destroy', $rate->id) }}" method="POST" style="display: inline;" onsubmit="if(confirm('Desea eliminar la Tarifa?')) { return true } else {return false };">
                          ...
                          @else                              
                            <div class="input-group-btn">
                                <button data-toggle="dropdown" class="btn btn-xs btn-danger dropdown-toggle" type="button" title="Aciones"><i class="fa fa-chevron-circle-down" aria-hidden="true"></i></button>
                                    <ul class="dropdown-menu">
                                        <li><a href="{{ route('rates.status', Crypt::encrypt($rate->id)) }}"><i class="fa fa-check"></i> Habilitar</a></li>
                                    </ul>
                            </div>
                          @endif
                        <!-- /Split button -->                          
